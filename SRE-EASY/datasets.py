...
def iterator(data, target, transforms, forcecpu=False, shuffle=True, use_hd=False):
    if args.dataset_device == "cpu" or forcecpu:
        ...
        return torch.utils.data.DataLoader(dataset, batch_size=args.batch_size, shuffle=shuffle,
                                           num_workers=min(6, os.cpu_count()), persistent_workers=True)
    else:
...
def CUBfs(use_hd=True):
    ...
    train_data = args.dataset_path + '/' + 'base.json'
    val_data = args.dataset_path + '/' + 'val.json'
    test_data = args.dataset_path + '/' + 'novel.json'
    ...
    for i, data in enumerate(data_list):
        if i == 0:
            with open(data, 'r') as f:
                ...
                count = patch[i]
                label_map_dict = {}
                for j in range(len(meta['image_labels'])):
                    if meta['image_labels'][j] not in label_map_dict:
                        label_map_dict[meta['image_labels'][j]] = count
                        meta['image_labels'][j] = count
                        count = count + 1
                    else:
                        meta['image_labels'][j] = label_map_dict[meta['image_labels'][j]]
                num_elements['train_base'] = list(Counter(meta['image_labels']).values())

                # for k in range(len(num_elements['train_base'])):
                #     index = k * 60
                #     if num_elements['train_base'][k] == 60:
                #         continue
                #     else:
                #         for m in range(60 - num_elements['train_base'][k]):
                #             meta['image_names'].insert(index, meta['image_names'][index])
                #             meta['image_labels'].insert(index, meta['image_labels'][index])
                datasets['train_base'] = [meta['image_names'], torch.LongTensor(meta['image_labels'])]
                # num_elements['train_base'] = list(Counter(meta['image_labels']).values())
        ...
        with open(data, 'r') as f:
            ...
            for j in range(len(meta['image_labels'])):
                if meta['image_labels'][j] not in label_map_dict:
                    ...
                    meta['image_labels'][j] = label_map_dict[meta['image_labels'][j]]
            num_elements[split[i]] = list(Counter(meta['image_labels']).values())
            for k in range(len(num_elements[split[i]])):
                index = k * 60
                if num_elements[split[i]][k] == 60:
                    continue
                else:
                    for m in range(60 - num_elements[split[i]][k]):
                        meta['image_names'].insert(index, meta['image_names'][index])
                        meta['image_labels'].insert(index, meta['image_labels'][index])

            datasets[split[i]] = [meta['image_names'], torch.LongTensor(meta['image_labels'])]
            num_elements[split[i]] = list(Counter(meta['image_labels']).values())
